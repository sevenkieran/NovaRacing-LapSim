...
% Modified by Kieran for NovaRacing Point Mass Lap Sim compatibility
...
% To create a new track from logged data, see Tracks/DataInput/
filename = "Tracks/DataInput/AutocrossTrackData.csv";
...
track_dir = 'Tracks';
[folder_status,folder_msg] = mkdir(track_dir) ;
trackname = track_dir+"/"+info.name+"_"+info.config+"_"+info.direction ;
...
max_radius = 10000; % Maximum radius in meters to represent a straight line
R = zeros(size(r));

% Safely invert curvature to get radius for actual corners
idx_curve = abs(r) >= (1/max_radius);
R(idx_curve) = 1 ./ r(idx_curve);

idx_straight = abs(r) < (1/max_radius);
% Preserve the turning direction (sign), but default to positive 1 if exactly 0
signs = sign(r(idx_straight));
signs(signs == 0) = 1; 
R(idx_straight) = max_radius .* signs;

track_table = table(X(:), Y(:), R(:), 'VariableNames', {'x', 'y', 'R'});


csv_filename = trackname + ".csv";
writetable(track_table, csv_filename);

% HUD Update
disp(['Track successfully exported to: ', char(csv_filename)]);
